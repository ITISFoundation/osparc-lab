qx.Class.define("app2.ui.SettingsView",
{
  ...
  members: {
    ...
    _buildForm : function() {
      ...
      if ("service" in this._model.getWorkbench().getSelected().getItem(0) &&
          "settings" in this._model.getWorkbench().getSelected().getItem(0).service) {
        var myServiceSetts = this._model.getWorkbench().getSelected().getItem(0).service.settings;

        if (myServiceSetts.length > 0) {
          var data = this._model.getWorkbench().getSelected().getItem(0).service.settings;
          ...
          computeButton.addListener("execute", function() {
            if (form.validate()) {
              for (var i = 0; i < this._model.getWorkbench().getSelected().getItem(0).service.settings.length; i++) {
                var settKey = this._model.getWorkbench().getSelected().getItem(0).service.settings[i].name;
                this._model.getWorkbench().getSelected().getItem(0).service.settings[i].value = model.get(settKey);
              }
            }
          }, this);
